...
def sentence1():
    ...
    A_or_B = A | B
    not_A_imp = (~A) % ((~B) | C)
    all_disjoin = disjoin(~A, ~B, C)
    return conjoin(A_or_B, not_A_imp, all_disjoin)
...
def sentence2():
    ...
    not_all = (~(B & (~C)) >> A)
    not_D = (~D) >> C
    return conjoin(C_imp, A_to, not_all, not_D)
...
def sentence3():
    ...
    """
    pac_alive_0 = PropSymbolExpr("PacmanAlive[0]")
    pac_alive_1 = PropSymbolExpr("PacmanAlive[1]")
    pac_born_0 = PropSymbolExpr("PacmanBorn[0]")
    pac_killed_0 = PropSymbolExpr("PacmanKilled[0]")
    first = pac_alive_1 % ((pac_alive_0 & (~pac_killed_0)) | ((~pac_alive_0) & (pac_born_0)))
    second = ~((pac_alive_0) & (pac_born_0))
    third = pac_born_0
...
def atMostOne(literals):
    ...
    """
    new = []
    litList = itertools.combinations(literals, 2)
    for lit in litList:
        item = disjoin(~lit[0], ~lit[1])
        new.append(item)
    return conjoin(new)
...
def pacphysics_axioms(t, all_coords, non_outer_wall_coords):
    ...
    for coord in all_coords:
        first = PropSymbolExpr(wall_str, coord[0], coord[1]) >> ~PropSymbolExpr(pacman_str, coord[0], coord[1], t)
        firstt.append(first)
    for coordd in non_outer_wall_coords:
        second = PropSymbolExpr(pacman_str, coordd[0], coordd[1], t)
        secondd.append(second)
    second_one = exactlyOne(secondd)
    for act in DIRECTIONS:
        third = PropSymbolExpr(act, t)
        thirdd.append(third)
    third_one = exactlyOne(thirdd)
    pacphysics_sentences.append(firstt)
    pacphysics_sentences.append(second_one)
    pacphysics_sentences.append(third_one)
...
def positionLogicPlan(problem):
    ...
    non_outerwall = []
    act_seq = []
    pac_startloc = PropSymbolExpr(pacman_str, x0, y0, 0)
    KB.append(pac_startloc)
    for t in range(50):
        for wall in non_wall_coords:
            pac_wall = PropSymbolExpr(pacman_str, wall[0], wall[1], t)
            non_wall.append(pac_wall)
        pac_exact = exactlyOne(non_wall)
        print(pac_exact)
        KB.append(pac_exact)
        goal = PropSymbolExpr(pacman_str, xg, yg, t)
        model = conjoin(KB, goal)
        if findModel(model):
            return extractActionSequence(model, actions)
        for act in actions:
            act_seq.append(PropSymbolExpr(act, t))
        get_act = exactlyOne(act_seq)
        KB.append(get_act)
        for wcoor in non_wall_coords:
            KB.append(pacmanSuccessorStateAxioms(wcoor[0], wcoor[1],t+1, walls))
