...
class ValueIterationAgent(ValueEstimationAgent):
            ...
            for st in self.mdp.getStates():
                if not self.mdp.isTerminal(st):
                    ...
                    for ac in self.mdp.getPossibleActions(st):
                        q_value = self.computeQValueFromValues(st, ac)
                        if q_value > best:
                            best = q_value
                        init[st] = best
            for state in self.mdp.getStates():
                self.values[state] = init[state]
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for stateProb in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            qval += (reward + self.getValue(stateProb[0]) * self.discount) * stateProb[1]
        return qval
